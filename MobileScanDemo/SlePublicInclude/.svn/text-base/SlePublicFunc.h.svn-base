...
#ifndef SLEPUBLICFUNC_H_
#define SLEPUBLICFUNC_H_
 ...
 * \date 		 	2010-03-16
 */
void HexStringToHex(char *pHexString, unsigned char *pHex, int iHexSize);


/*! \fn void HexStringToHexArray(char *pHexString, unsigned char *pHex)
 * \brief
 * \param[in] 	pHexString 		---010D2303
 * \param[out] 	pHex 			---pHex[0]=0x01; pHex[1]=0x13; pHex[2]=0x35; pHex[3]=0x03;
 * \return 	 	void
 * \author 		wangyzh
 * \date 		2014-09-18
 */
void HexStringToHexArray(char *pHexString, unsigned char *pHex);
...
int CovertDateStringToCustDateString(unsigned char * DateStrInput,
		const char *FormartStrOutput, unsigned char *DateStrOutput,unsigned int DateStrOutputLength);

time_t PubGetNowTime();

int  CovertDateStringToCustDateString(unsigned char * TimeStrInput,
		const char *FormartStrOutput, unsigned char *TimeStrOutput,unsigned int TimeStrOutputLength);

int CovertTimeStringToCustTimeString(unsigned char * TimeStrInput,
		const char *FormartStrOutput, unsigned char *TimeStrOutput,unsigned int TimeStrOutputLength);
...
//产品化2014
int SemTimedwait(sem_t * pSem, int iTimeoutInMs);
void HexToStringForLog(const BYTE * szSrcData, int iSrcDataLength, char * szDesString, int iDesBufSize);

/*! \fn 		GetIntervalRunDateList
* \brief        获取两个运营日间隔的运营日期列表
* \param[in]
* \param[out]   无
* \return
* \author		wangzp
* \date			2015-04-11
*/
void GetIntervalRunDateList(const char * pCurrentRunDate, const char * pLastRunDate, char ** pCalDate, int *piCount);

//产品化 End

#endif /* SLEPUBLICFUNC_H_ */
